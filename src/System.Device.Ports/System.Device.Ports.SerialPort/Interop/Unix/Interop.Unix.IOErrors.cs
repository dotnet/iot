...
namespace System.Device.Ports.SerialPort
{
    internal static partial class Interop
    {
        ...
        {
            Debug.Assert(errorInfo.Error != Error.SUCCESS, "Error encountered");
        ...
        internal static Exception GetExceptionForIoErrno(ErrorInfo errorInfo, string? path = null, bool isDirectory = false)
        {
            ...
            switch (errorInfo.Error)
            {
                case Error.ENOENT:
                    if (isDirectory)
                    {
                        return !string.IsNullOrEmpty(path) ?
                            new DirectoryNotFoundException(ISR.Format(Strings.IO_PathNotFound_Path, path)) :
                            new DirectoryNotFoundException(Strings.IO_PathNotFound_NoPathName);
                    }
                    else
                    {
                        return !string.IsNullOrEmpty(path) ?
                            new FileNotFoundException(ISR.Format(Strings.IO_FileNotFound_FileName, path), path) :
                ...
                case Error.EPERM:
                    ...
                    return !string.IsNullOrEmpty(path) ?
                        new UnauthorizedAccessException(ISR.Format(Strings.UnauthorizedAccess_IODenied_Path, path), inner) :
                        new UnauthorizedAccessException(Strings.UnauthorizedAccess_IODenied_NoPathName, inner);

                case Error.ENAMETOOLONG:
                    return !string.IsNullOrEmpty(path) ?
                        new PathTooLongException(ISR.Format(Strings.IO_PathTooLong_Path, path)) :
                        new PathTooLongException(Strings.IO_PathTooLong);

                case Error.EWOULDBLOCK:
                    return !string.IsNullOrEmpty(path) ?
                        new IOException(ISR.Format(Strings.IO_SharingViolation_File, path), errorInfo.RawErrno) :
                ...
                case Error.EEXIST:
                    ...
                    {
                        return new IOException(ISR.Format(Strings.IO_FileExists_Name, path), errorInfo.RawErrno);
                    }
