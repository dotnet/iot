...
namespace System.Device.Ports.SerialPort.Tests
{
    public class ReceivedBytesThreshold_Property : PortsTest
    {
        ...
        private class ReceivedEventHandler
        {
            ...
            private readonly List<SerialPort> _source = new List<SerialPort>();
            ...
            public void Validate(SerialData eventType, int bytesToRead, int eventIndex)
            {
                lock (this)
                {
                    ...
                    if (_com != _source[eventIndex])
                    {
                        Fail("ERROR!!! Expected {0} source actual={1}", _com.BaseStream, _source[eventIndex]);
                    }
                }
            }
