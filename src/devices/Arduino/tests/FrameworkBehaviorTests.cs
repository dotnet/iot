...
namespace Iot.Device.Tests
{
    public class FrameworkBehaviorTests
    {
        ...
        public void DoubleToString2()
        {
            ...
            Dragon4Double(d, 2, false, buffer, out int exponent);
            byte[] expected = new byte[]
            {
                (byte)'2', (byte)'0', (byte)'2', (byte)'3'
            };

            Assert.Equal(1, exponent);
            Assert.Equal(expected, buffer.ToArray().Where(x => x != 0));
        }
        ...
        public static int Dragon4Double(double value, int cutoffNumber, bool isSignificantDigits, Span<byte> number, out int decimalExponent)
        {
            ...
            int length = (int)(Dragon4(mantissa, exponent, mantissaHighBitIdx, hasUnequalMargins, cutoffNumber, isSignificantDigits, number, out decimalExponent));
            return length;
        }
        ...
        private static unsafe uint Dragon4(ulong mantissa, int exponent, uint mantissaHighBitIdx, bool hasUnequalMargins, int cutoffNumber, bool isSignificantDigits, Span<byte> buffer, out int decimalExponent)
        {
            ...
            Debug.Assert(buffer.Length > 0, "Buffer is empty");
            ...
            Debug.Assert(mantissa != 0, "Value is 0?");
        ...
        internal unsafe ref struct BigInteger
        {
            ...
            public static void DivRem(ref BigInteger lhs, ref BigInteger rhs, out BigInteger quo, out BigInteger rem)
            {
                ...
                {
                    uint quotient = DivRem(lhs._blocks[0], rhs._blocks[0], out uint remainder);
                ...
                if (rhsLength == 1)
                {
                    ...
                    for (int i = quoLength - 1; i >= 0; i--)
                    {
                        ...
                        ulong digit = DivRem(value, rhsValue, out carry);
            ...
            public static void Multiply(ref BigInteger lhs, ref BigInteger rhs, out BigInteger result)
            {
                ...
                Unsafe.InitBlockUnaligned((byte*)result.GetBlocksPointer(), 0, (uint)maxResultLength * sizeof(uint));
            ...
            public static void Pow2(uint exponent, out BigInteger result)
            {
                ...
                if (blocksToShift > 0)
                {
                    Unsafe.InitBlockUnaligned((byte*)result.GetBlocksPointer(), 0, blocksToShift * sizeof(uint));
                }
            ...
            public static void SetValue(out BigInteger result, ref BigInteger value)
            {
                ...
                Unsafe.CopyBlock((byte*)result.GetBlocksPointer(), (byte*)value.GetBlocksPointer(), (uint)rhsLength * sizeof(uint));
            }
            ...
            public void ShiftLeft(uint shift)
            {
                ...
                if (remainingBitsToShift == 0)
                {
                    ...
                    Unsafe.InitBlockUnaligned((byte*)GetBlocksPointer(), 0, blocksToShift * sizeof(uint));
                }
                else
                {
